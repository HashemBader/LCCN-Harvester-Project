        ...
        try:
            if self.targets_file.exists():
                ...
                for target in targets:
                    name = self.target_aliases.get(target.get("name"), target.get("name"))
                    if name in self.api_cards:
                        enabled = target.get("selected", True)
                        self.api_cards[name].set_enabled(enabled)
        except Exception as e:
